...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("the number of formals and vals should be the same")
...
def do_and_form(expressions, env):
    ...
    expr = expressions
    while expr != nil:
        cur_res = scheme_eval(expr.first, env, True if expr.rest == nil else None)
        if is_false_primitive(cur_res) or expr.rest == nil:
            return cur_res
        expr = expr.rest
    return True
    # END PROBLEM 12
