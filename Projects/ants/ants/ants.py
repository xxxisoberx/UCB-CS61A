...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dis_to_ori_place = 0        
        if dis_to_ori_place < self.min_range:
            for _ in range(self.min_range):
                if check_place.entrance == beehive:
                    return None
                check_place = check_place.entrance
            dis_to_ori_place = self.min_range

        while check_place is not beehive and self.min_range <= dis_to_ori_place and dis_to_ori_place <= self.max_range:
            if check_place.bees:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            bee_with_same_place = [b for b in gamestate.bees if b.place is self.place]
            if bee_with_same_place:
                self.eat_bee(random.choice(bee_with_same_place))
        # END Problem 6
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        bee_with_same_place = [b for b in gamestate.bees if b.place is self.place]
        for bee in bee_with_same_place:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.queen:
            ...
            place_behind_queen, p = [], self.place.exit
            while p:
                place_behind_queen.append(p)
                p = p.exit
            bee_behind_queen = [b for b in gamestate.ants if b.place in place_behind_queen]
            for bee in bee_behind_queen:
                if isinstance(bee, ContainerAnt):
                    if bee.contained_ant and not bee.contained_ant.buff:
                        bee.contained_ant.damage *= 2
                        bee.contained_ant.buff = True
                if not bee.buff:
                    bee.damage *= 2
                    bee.buff = True
        else:
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    new_status = status(bee.action, bee)
    old_status = bee.action

    def new_action(gamestate):
        nonlocal length
        if length > 0:
            new_status(gamestate)
            length -= 1
        else:
            old_status(gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while check_place is not beehive:
            if check_place.ant and not isinstance(check_place.ant, LaserAnt):
                insects_and_distances[check_place.ant] = current_distance
                if isinstance(check_place.ant, ContainerAnt) and check_place.ant.contained_ant and not isinstance(check_place.ant.contained_ant, LaserAnt):
                    insects_and_distances[check_place.ant.contained_ant] = current_distance
            for bee in check_place.bees:
                insects_and_distances[bee] = current_distance
            check_place = check_place.entrance
            current_distance += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        damage_dec_by_dis = 0.2 * distance
        damage_dec_by_num_insec = 0.05 * self.insects_shot
        cur_damage = max(0, self.original_damage-damage_dec_by_dis-damage_dec_by_num_insec)
        if cur_damage:
            self.damage = self.damage - damage_dec_by_num_insec
        return cur_damage
        # END Problem OPTIONAL
