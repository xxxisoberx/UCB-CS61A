...
def reverse_iter(lst):
    ...
    "*** YOUR CODE HERE ***"
    length = len(lst)
    return [lst[length - 1 - i] for i in range(length)]
...
def reverse_recursive(lst):
    ...
    if length == 1:
        ...
        seq = [lst[length - 1]]
        lst = [lst[i] for i in range(length - 1)]
        return seq + reverse_recursive(lst)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a = distance(city_a, destination)
    dis_b = distance(city_b, destination)
...
def add_chars(w1, w2):
    ...
    """
    以下代码是正确答案，但：
    如果a = 'abc', 则a[3:], a[4:], a[5:] …… a[100:] 等表达式都是合法的，且它们的值都为''

    if w1[0] == w2[0]:
        if len(w1) == 1:
            return '' if len(w2) == 1 else w2[1:]
        else:
