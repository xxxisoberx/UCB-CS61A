...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skeleton_idx == len(skeleton) and code_idx < len(code):
            ...
            return edits, len(skeleton) - code_idx
        
        possibilities = []
        skel_char, code_char = skeleton[skeleton_idx], code[code_idx]
        # Match
        if skel_char == code_char:
            rest_match, rest_cost = helper_align(skeleton_idx+1, code_idx+1)
            # _________________________________________
            possibilities.append((code_char + rest_match, rest_cost))
        # Insert
        rest_match, rest_cost = helper_align(skeleton_idx, code_idx+1)
        # _________________________________________
        possibilities.append(("+[" + code_char + "]" + rest_match, 1 + rest_cost))
        # Delete
        rest_match, rest_cost = helper_align(skeleton_idx+1, code_idx)
        # _________________________________________
        possibilities.append(("-[" + skel_char + "]" + rest_match, 1 + rest_cost))
...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    count = 0
    def helper(s_left, first_sum, second_sum):
        nonlocal count
        if not s_left and abs(first_sum - second_sum) <= d:
            count += 1
        elif s_left:
            helper(s_left[1:], first_sum + s_left[0], second_sum)
            helper(s_left[1:], first_sum, second_sum + s_left[0])
    helper(s, 0, 0)  
    return count // 2
