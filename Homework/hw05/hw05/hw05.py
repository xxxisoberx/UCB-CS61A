...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, p):
        ...
        if len(incorrect_password) == 3:
            return "Too many incorrect attempts. Attempts: " + str(incorrect_password)
        if p == password:
            if amount <= balance:
                balance -= amount
                return balance
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    wd_output = withdraw(0, old_pass)
    if type(wd_output) == str:
        return wd_output
    def new_withdraw(amount, password):
        return withdraw(amount, old_pass) if password == new_pass or password == old_pass else withdraw(amount, password)
    return new_withdraw
