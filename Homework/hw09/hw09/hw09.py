...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(i, res, term_pre):
        term_cur = even_term if term_pre == odd_term else odd_term
        res += term_cur(i)
        return res if i == n else helper(i+1, res, term_cur)
    return helper(1, 0, even_term)
...
def mutate_reverse(link):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(link_left, cur_link):
        if link_left == Link.empty:
            return cur_link
        return helper(link_left.rest, Link(link_left.first, cur_link))
    temp = helper(link, Link.empty)
    link.first = temp.first
    link.rest = temp.rest
